...
pub async fn main(req: Request, env: Env, _ctx: Context) -> Result<Response> {
    ...
    let request_id = req.headers().get("cf-ray").unwrap_or_else(|_| Some(uuid::Uuid::new_v4().to_string())).unwrap_or_else(|| uuid::Uuid::new_v4().to_string());
    ...
    headers.set("X-Response-Time", &format!("{}ms", duration)).ok();
