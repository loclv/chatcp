...
fn api_error(msg: &str) -> Result<Response> {
    let resp = ApiResponse::<()>::error(msg);
    Response::from_json(&resp).map(|r| r.with_status(400))
}

fn not_found(msg: &str) -> Result<Response> {
    let resp = ApiResponse::<()>::error(msg);
    Response::from_json(&resp).map(|r| r.with_status(404))
}

fn internal_error(e: impl std::fmt::Display) -> Result<Response> {
    console_error!("Internal error: {}", e);
    let resp = ApiResponse::<()>::error("Internal server error");
    Response::from_json(&resp).map(|r| r.with_status(500))
}
...
pub async fn create_agent(d1: &D1Database, req: &CreateAgentRequest) -> Result<Response> {
    ...
    let owner_id: D1Value = match &req.owner_id {
    ...
    let result = d1
        ...
        .bind(&[
            ...
            req.description.as_deref().unwrap_or("").into(),
            owner_id,
        ])?
        .run()
        .await;
    ...
    match result {
        Ok(_) => match get_agent_by_id(d1, &id).await? {
            ...
            None => internal_error("Failed to retrieve created agent"),
        },
        Err(e) => internal_error(format!("Failed to create agent: {}", e)),
    }
}
...
pub async fn list_agents(d1: &D1Database) -> Result<Response> {
    ...
    match result {
        ...
        Err(e) => internal_error(format!("Failed to list agents: {}", e)),
    }
}
...
pub async fn update_agent(d1: &D1Database, id: &str, req: &UpdateAgentRequest) -> Result<Response> {
    let existing = match get_agent_by_id(d1, id).await? {
        Some(a) => a,
        None => return not_found(&format!("Agent '{}' not found", id)),
    };
    ...
    let result = d1
        ...
        .bind(&[name.into(), description.into(), owner_id, id.into()])?
        .run()
        .await;

    match result {
        Ok(_) => get_agent(d1, id).await,
        Err(e) => internal_error(format!("Failed to update agent: {}", e)),
    }
}
    ...
    match result {
        Ok(meta) => {
            if meta.changes() > 0 {
                ...
                not_found(&format!("Agent '{}' not found", id))
            }
        }
        Err(e) => internal_error(format!("Failed to delete agent: {}", e)),
    }
}
...
pub async fn create_owner(d1: &D1Database, req: &CreateOwnerRequest) -> Result<Response> {
    ...
    match result {
        Ok(_) => match get_owner_by_id(d1, &id).await? {
            ...
            None => internal_error("Failed to retrieve created owner"),
        },
        Err(e) => {
            let msg = format!("{}", e);
            if msg.contains("UNIQUE") {
                return api_error("An owner with this email already exists");
            }
            internal_error(format!("Failed to create owner: {}", e))
        }
    }
}
...
pub async fn list_owners(d1: &D1Database) -> Result<Response> {
    ...
    match result {
        ...
        Err(e) => internal_error(format!("Failed to list owners: {}", e)),
    }
}
...
pub async fn get_owner(d1: &D1Database, id: &str) -> Result<Response> {
    match get_owner_by_id(d1, id).await {
        ...
        Ok(None) => not_found(&format!("Owner '{}' not found", id)),
        Err(e) => internal_error(e),
    }
}
...
pub async fn update_owner(d1: &D1Database, id: &str, req: &UpdateOwnerRequest) -> Result<Response> {
    let existing = match get_owner_by_id(d1, id).await? {
        Some(o) => o,
        None => return not_found(&format!("Owner '{}' not found", id)),
    };
    ...
    match result {
        ...
        Err(e) => internal_error(format!("Failed to update owner: {}", e)),
    }
}
    ...
    match result {
        Ok(meta) => {
            if meta.changes() > 0 {
                ...
                not_found(&format!("Owner '{}' not found", id))
            }
        }
        Err(e) => internal_error(format!("Failed to delete owner: {}", e)),
    }
}
...
pub async fn create_chat(d1: &D1Database, req: &CreateChatRequest) -> Result<Response> {
    ...
    match result {
        Ok(_) => match get_chat_by_id(d1, &id).await? {
            ...
            None => internal_error("Failed to retrieve created chat"),
        },
        Err(e) => internal_error(format!("Failed to create chat: {}", e)),
    }
}
...
pub async fn list_chats(d1: &D1Database) -> Result<Response> {
    ...
    match result {
        ...
        Err(e) => internal_error(format!("Failed to list chats: {}", e)),
    }
}
...
pub async fn get_chat_with_messages(d1: &D1Database, id: &str) -> Result<Response> {
    let chat = match get_chat_by_id(d1, id).await? {
        Some(c) => c,
        None => return not_found(&format!("Chat '{}' not found", id)),
    };
    ...
    match messages_result {
        ...
        Err(e) => internal_error(format!("Failed to get messages: {}", e)),
    }
}
...
pub async fn update_chat(d1: &D1Database, id: &str, req: &UpdateChatRequest) -> Result<Response> {
    ...
    match result {
        Ok(_) => {
            // Return the full chat with messages since the GET endpoint returns that
            get_chat_with_messages(d1, id).await
        }
        Err(e) => internal_error(format!("Failed to update chat: {}", e)),
    }
}
    ...
    match result {
        Ok(meta) => {
            if meta.changes() > 0 {
                ...
                not_found(&format!("Chat '{}' not found", id))
            }
        }
        Err(e) => internal_error(format!("Failed to delete chat: {}", e)),
    }
}
...
pub async fn send_message(d1: &D1Database, chat_id: &str, req: &SendMessageRequest) -> Result<Response> {
    ...
    if get_chat_by_id(d1, chat_id).await?.is_none() {
        return not_found(&format!("Chat '{}' not found", chat_id));
    }

    // Validate sender_type
    if req.sender_type != "agent" && req.sender_type != "owner" {
        return api_error("sender_type must be 'agent' or 'owner'");
    }
    ...
    match result {
        Ok(_) => {
            ...
            match msg {
                ...
                None => internal_error("Failed to retrieve created message"),
            }
        }
        Err(e) => internal_error(format!("Failed to send message: {}", e)),
    }
}
